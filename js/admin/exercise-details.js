...
function openNotesModal() {
    ...
    textarea.value = currentExercise.notes || '';
...
async function saveNotes(e) {
    ...
    try {
        ...
        if (notes) {
            ...
            notesContent.innerHTML = '<p class="no-data">Nenhuma observação registrada</p>';
        }
...
function displayExerciseDetails() {
    ...
    document.getElementById('editNotesBtn').addEventListener('click', openNotesModal);

    // Exibir observações
    const notesContent = document.getElementById('notesContent');
    if (exercise.notes && exercise.notes.trim()) {
        notesContent.innerHTML = `<p>${exercise.notes.replace(/\n/g, '<br>')}</p>`;
    } else {
        notesContent.innerHTML = '<p class="no-data">Nenhuma observação registrada</p>';
    }
}
